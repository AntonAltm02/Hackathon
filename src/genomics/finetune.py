...
hyperparameter_defaults = dict(
    ...
    dataset_name="P24_V5",
    do_train=True,
    load_model="../scGPT_finetune_org_vocab",
    mask_ratio=0.4,
    epochs=25,
    n_bins=51,
...
if dataset_name == "PBMC_10K":
    ...
    ori_batch_col = "batch"
    adata.obs["timepoint"] = adata.obs["str_labels"].astype("category")
    ...
    data_is_raw = False
    # IMPORTANT CHOICE; matching mouse genes to human genes to use pretrained knowledge
...
adata.var["gene_name"] = adata.var["feature_name"].str.upper().tolist()
print('###: -1')
...
zero_rows = np.sum(adata.X, axis=1) == 0
print("Zero rows:", np.any(zero_rows))
print("Number of zero rows:", np.sum(zero_rows))
...
preprocessor = Preprocessor(
    ...
    result_normed_key="normalized",  # the key in adata.layers to store the normalized data
