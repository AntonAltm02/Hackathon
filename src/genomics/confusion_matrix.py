...
def get_cancer_score(search_embedding, reference_embeddings, labels, k=25):
...
reference_embeddings = np.load('reference_P21.npy')
...
cell_type_key = "cell_type_fit"
batch_key = "SCT_snn_res.0.4"
N_HVG = 3000
...
for i, search_embedding in enumerate(embed_adata.obsm["X_scGPT"]):
    ...
    ground_truth.append(int(embed_adata.obs['SCT_snn_res.0.4'].values[i]))
...
conf_matrix_df = pd.DataFrame(conf_matrix,
                             index=[f"ENC{label}" for label in labels],
                             columns=[f"ENC{label}" for label in labels])
import matplotlib.pyplot as plt
...
# Create and save the heatmap for the confusion matrix
...
plt.savefig(rf'confusion_matrix_{NAME}.png', dpi=300)
...
plt.savefig(rf'confusion_matrix_percentage_{NAME}.png', dpi=300, bbox_inches='tight')