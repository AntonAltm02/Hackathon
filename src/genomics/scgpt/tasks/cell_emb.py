...
def find_gene_name(gene_id, vocab):
    vocab_dict = vocab.get_stoi()
    reverse_vocab = {v: k for k, v in vocab_dict.items()}
    gene_name = reverse_vocab.get(gene_id)
    # print(f"Gene ID {gene_id} corresponds to gene: {gene_name}")
    return gene_name


def find_gene_name_cached(gene_id, vocab, cache):
    if gene_id in cache:
        return cache[gene_id]
    gene_name = find_gene_name(gene_id, vocab)
    cache[gene_id] = gene_name
    return gene_name


def create_global_padding_mask(data_loader, vocab, pad_token):
    all_gene_ids = torch.cat([data_dict['gene'] for data_dict in data_loader], dim=0)
    global_padding_mask = all_gene_ids.eq(vocab[pad_token])
    return global_padding_mask
...
def get_batch_cell_embeddings(
    ...
    if cell_embedding_mode == "cls":
        ...
        np.save(f'embeddings/merged_finetuned2_scgpt.npy', cell_embeddings)
        return  cell_embeddings

    elif cell_embedding_mode == "pad": #GENES
        ...
        num_genes = max_length-1
        embeddings_all = []
        # global_padding_mask = create_global_padding_mask(data_loader, vocab, model_configs['pad_token'])
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(enabled=True):
            ...
            for data_dict in tqdm(data_loader, desc="Embedding cells"):
                ...
                gene_ids = input_gene_ids.detach().cpu().numpy()
                # print(global_padding_mask)
                # print(global_padding_mask.shape)
                # print(f'Input shape: {input_gene_ids.shape}')
                embeddings = model._encode(
                    input_gene_ids,
                    data_dict["expr"].to(device),
                    src_key_padding_mask=src_key_padding_mask,
                ...
                # LOOP
                # for idx, gene_id in enumerate(gene_ids[0][1:], 1):  # Skip CLS token (index 0)
                #     if gene_id == 60695 or gene_id == 60694:
                #         continue
                #
                #     gene_name = find_gene_name(gene_id, vocab)
                #     gene_embedding = embedding[0, idx, :]
                #
                #     tmp = (cell_id, gene_name, gene_embedding)
                #     embeddings_all.append(tmp)

                # LIST COMPREHENSION
                embeddings_all.extend([
                    (cell_id, find_gene_name_cached(gene_id, vocab, gene_name_cache), embedding[0, idx, :])
                    for idx, gene_id in enumerate(gene_ids[0][1:], 1)
                    if gene_id not in {60695, 60694}
                ])
        ...
        # cell_embeddings = cell_embeddings_all / np.linalg.norm(cell_embeddings_all, axis=1, keepdims=True)
        os.makedirs('embeddings', exist_ok=True)
        with open(f'embeddings/gene_embeddings_{max_length}.pkl', 'wb') as f:
            pickle.dump(embeddings_all, f)

        # # Optional: Save with compression to save space
        # np.savez_compressed('embeddings/final_embeddings_compressed.npz', embeddings=embeddings_all)
...
def embed_data(
    ...
    # print(adata.var["id_in_vocab"])
    # missing_genes = adata.var[adata.var['id_in_vocab'] == -1].index.tolist()
    # with open('missing_genes.txt', 'w') as file:
    #     for gene in missing_genes:
    #         file.write(gene + '\n')

    gene_ids_in_vocab = np.array(adata.var["id_in_vocab"])
    if max_length != 1200:
        max_length = np.sum(gene_ids_in_vocab >= 0) + 1
    logger.info(
